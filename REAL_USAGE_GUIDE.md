# All Day Poke - Real Usage Data Guide

## How to Display Your Real Claude Usage in All Day Poke

Since Claude Code requires authentication, All Day Poke provides multiple ways to display your real usage data:
...
2. **Update All Day Poke with your usage percentage:**
   ```bash
...
3. **The pet will immediately show your real usage!**
   - The data is saved in `~/.alldaypoke/real-usage.json`
...
- Make sure the file `~/.alldaypoke/real-usage.json` exists
...
alias update-alldaypoke='claude /usage && echo "Enter percentage:" && read pct && node ~/desktop_bot/manual-usage-update.js $pct'
```

Then just run `update-alldaypoke` anytime to update your usage.

## Architecture

The app checks for real usage data in this priority order:
1. Manual usage file (`~/.alldaypoke/real-usage.json`)
...
- `~/.alldaypoke/real-usage.json` - Stored real usage data
- `~/.alldaypoke/config.json` - App configuration including auth settings